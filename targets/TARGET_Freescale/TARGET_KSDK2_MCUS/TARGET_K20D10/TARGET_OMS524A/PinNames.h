...
typedef enum {
	...
	// Nanotron pin mapping (SPI2)
	NT_SPI_SCK = PTD12,
	NT_SPI_MOSI = PTD13,
	NT_SPI_MISO = PTD14,
	NT_SPI_CS = PTD11,
	NT_INT = PTB20,			// input
	NT_TMR_INT = PTA10, 	// input to FTM2
	NT_TMR_TXRX = PTA11,	// input to FTM2
	NT_FCLK = PTA18, 		// Clock input as FTM_CLK0
	NT_RESET = PTB21,		// output
	NT_WAKE = PTB22,		// output used to wake chip up
	NT_AWAKE = PTC5, 		// input, LLWU_P9
	
	// Antenna controls - not used
	LNA_ENABLE = PTB6, 		// output
	ANT_SELECT = PTB7,		// output
	...
	// Serial flash, SPI0 shared with DW
	EE_SPI_SCK = PTD1,
	EE_SPI_MOSI = PTD2,
	EE_SPI_MISO = PTD3,
	EE_SPI_CS = PTD5,
	...
	// DW1000 pin mapping (SPI0 shared with EE serial flash)
	DW_SPI_SCK = PTD1,
	DW_SPI_MOSI = PTD2,
	DW_SPI_MISO = PTD3,
	DW_SPI_CS = PTD0,
	DW_INT = PTC6, 			// input, LLWU_P10
	DW_CNTRL = PTC9, 		// output
	DW_RESET = PTE6, 		// output
	
	// BLE, CC2640
	BLE_RX = PTC14,
	BLE_TX = PTC15,
	BLE_RESET = PTA29, 		// output
	BLE_INT = PTB0, 		// input, LLWU_P5
	BLE_WAKE = PTC12,		// output (also RTS)
	BLE_STATUS = PTC13,		// input (also CTS)
